...
## [8.13.1] - 2026-09-07 — האיטיות: מדידה, ומה שהיא מצאה

### Fixed
- **הדופק הירוק של "בזמן" שרף 33% מהמעבד ברקע** (`index.css`): `.pulsing-dot` הנפיש `box-shadow` — תכונת **ציור**, לא תכונת קומפוזיטור — בלולאה **אינסופית**, על נקודה אחת לכל כרטיס טיסה, כל עוד הטאב פתוח. כלומר הדף צייר את עצמו מחדש ~60 פעם בשנייה בלי שאף אחד נגע בו. **נמדד: 989ms של עבודה על פני 3 שניות במנוחה מוחלטת, ו-182 חישובי סגנון.** אחרי המעבר לפסאודו-אלמנט שרק מתרחב ודוהה (`transform` + `opacity` בלבד, מורכבים ב-GPU בלי ציור): **95ms ו-15 חישובי סגנון** — פי עשרה פחות. זה טאב ברירת המחדל של האפליקציה, וציור רציף מרעיב כל אינטראקציה אחרת — כולל רשימת הימים.
- **כרטיס יום מוזכר (`React.memo`)** (`PlanningTab.jsx`): כיווץ יום רינדר מחדש את כל שאר הימים איתו, כי המצב יושב ב-`PlanningTab`. הפרופיילר הצביע על `setValueForStyle` בראש הרשימה — לא הלוגיקה של הקומפוננטה, אלא React כותב סגנונות inline בחזרה על צמתים שלא השתנו. הקוד של הכרטיס (478 שורות) הועבר כמו שהוא לקומפוננטה ממוזכרת.
  - **הניסיון הראשון לא שינה כלום, וזה נמדד**: העברתי את ה-`Set`ים עצמם כ-props, וכל כרטיס קיבל `Set` חדש בכל החלפה — המזכר מעולם לא תפס. עכשיו עוברים **בוליאנים לכל יום** (`isSummary`, `dayBodyShown`, `isEditing`, `isArrived`), כך שרק היום שנגעו בו משתנה. הסקריפט ירד מ-~48ms ל-~35ms.
  - שמונה פונקציות שהכרטיס מקבל חייבות להיות יציבות בזהות, אחרת המזכר קישוט: שתיים שנקראות בזמן רינדור עברו ל-`useCallback`, ושש שנקראות רק מאירועים עוברות דרך ref שמחזיק את המימוש העדכני — יציבות לתמיד, בלי רשימות תלויות ובלי סכנת closure מיושן.

### Notes on what the measurements actually showed
- ה-118ms שמדדתי בהתחלה היו **ארטיפקט של שרת הפיתוח**; בבילד ייצור אין שם long task בכלל. כל המדידות כאן הן על בילד ייצור עם האטת מעבד ×6.
- אחרי המזכר, סך המשימה לכיווץ יום ירד רק במעט (‎~145ms → ~130ms‎): רוב הזמן שם אינו JavaScript אלא ציור והרכבה של רשימה שמשנה גובה. השיפור הגדול באמת הוא הדופק, שהשפיע על **כל** האפליקציה ולא רק על הרשימה הזו.

## [8.13.0] - 2026-09-07 — מתג תת-הטאבים, פתיחת יום, וחלוניות שנסגרות בהחלקה

### Changed
- **מתג תת-הטאבים עוצב מחדש, עם אינדיקטור שנוסע** (`.subtab-switch`): אותו רעיון כמו שורת הניווט התחתונה — לוח אחד שנע בין שתי התוויות במקום רקע שמהבהב מאחת לשנייה, כך שהעין עוקבת אחרי התנועה ונוחתת על המקום שבו היא נמצאת. הטאב הפעיל מקבל משטח מורם, אייקון באקצנט וטקסט ראשי; הלא-פעיל שקט.
- **התוויות קוצרו ל"מקומות" ו"לוח זמנים"** — זה הפתרון לשבירת השורה. עם התוויות הארוכות הגלולה איבדה מספיק רוחב כדי ש"אטרקציות ומקומות" ירד לשורה שנייה והפקד כולו גדל בשורה. האייקונים נושאים את שאר המשמעות.

### Fixed
- **פתיחה וסגירה של יום נראתה תקועה.** הפרופיילר היה שקט, וזו הייתה הנקודה: ב**בילד ייצור** אין בכלל long task בפעולה הזו (ה-118ms שמדדתי בהתחלה היו ארטיפקט של שרת הפיתוח). המחיר לא היה ב-JavaScript אלא בקומפוזיטור — **לכל `.glass-card` היה `backdrop-filter: blur(16px)`**, ורשימת ימים מעלה תריסר משטחים מטושטשים למסך בבת אחת; פתיחת יום אחד מזיזה את כל אלה שמתחתיו ומאלצת את הדפדפן לדגום ולטשטש כל אחד מהם מחדש. הטשטוש הוסר מכרטיסי הרשימה ונשאר איפה שהוא באמת עובד — הכותרת, שורת הניווט והחלוניות, שצפות מעל תוכן שנגלל מתחתיהן. המילוי הועבה מעט לפיצוי.
  - **גוף היום נכנס בהנפשה** (`opacity` + הזזה קטנה בלבד — הנפשת גובה הייתה מכניסה layout לכל פריים, בדיוק הסוג היקר).
  - **הערה על מה שלא נמדד**: הסביבה כאן לא מרכיבה `backdrop-filter` בכלל (headless מדווח 0 משטחים מטושטשים), ולכן את השיפור עצמו לא יכולתי למדוד — הזיהוי מבוסס על כך שהעלות אינה ב-JS ועל מספר המשטחים המטושטשים במסך.
- **החץ של היום דיווח מצב שגוי**: הוא הראה "סגור יום" גם על יום מכווץ, כי הכיתוב נגזר ממצב תצוגת הסיכום במקום מהשאלה אם הגוף על המסך.
- **החלקה למטה לסגירה בכל החלוניות**: הוספה ל"הוספת פעילות ליום" (הגדולה בטאב, והיחידה שלא הייתה אפשר לסגור בהחלקה), ל"שיתוף וניהול חברי הטיול" ול"רשימת ציוד קבועה". התוכן שלהן לא נגע — רק נוספה הידית והמחווה.
  - שתי מלכודות hooks תוקנו תוך כדי: ה-hook של חלונית הפעילות קרא ל-`activityFormDirty` לפני שהוגדרה (temporal dead zone שהפילה את הטאב כולו), ושל רשימת הציוד ישב **אחרי** `if (!isOpen) return null` — כלומר hook שנוסף ברגע שהחלונית נפתחת.
  - דיאלוגים ממורכזים (אישור מחיקה, `ConfirmModal`, טיפים בממיר המטבעות) לא קיבלו החלקה בכוונה: הם החלטה קצרה במרכז המסך ולא חלונית תחתית, והמחווה לא שייכת להם.

## [8.12.1] - 2026-09-07 — שורת הכלים של לוח הזמנים ירדה

### Changed
- **הכותרת "לוח זמנים לפי ימים" הוסרה** — היא חזרה על מה שכפתור תת-הטאב שמעליה כבר אומר.
- **"הוסף יום" עבר לכפתור הצף**, בדיוק כמו "יעד חדש" ברשימת המקומות. הוספת יום היא פעולה של תת-הטאב כולו; הוספת *פעילות* עדיין שייכת ליום מסוים ונשארת על הכרטיס שלו.
- **מתג התצוגה עבר לצד בורר תת-הטאבים** — לא צף, ולא בתוך הגלולה: הגלולה היא לבחירת תת-הטאב, וסגמנט שלישי בתוכה היה נקרא כטאב שלישי. לצידה הוא נקרא כמה שהוא — שליטה על הרשימה שמתחת.
- **שורת הכלים של לוח הזמנים נשארת רק כשיש בה משהו** (הכפתור "ייצר/עדכן ימים לפי טיסה", או אישור הסנכרון). כשהימים מסונכרנים היא לא מרונדרת בכלל.

**התוצאה, נמדד:** היום הראשון מתחיל ב-**159px** מראש המסך במקום 225px, וכשהתצוגה על סיכום — **חמישה ימים שלמים נראים בלי לגלול בכלל**.

## [8.12.0] - 2026-09-06 — יום חדש שרואים, ותצוגת סיכום ללוח הזמנים

### Added
- **הוספת יום מגלגלת אליו ומדגישה אותו** (`PlanningTab.jsx`): יום נוסף לתחתית הרשימה, שבטיול באורך סביר נמצאת מחוץ למסך — ולכן ההוספה נראתה כאילו לא קרה כלום (נמדד: "יום 5" ישב 1,566px מתחת לקצה העליון). **בחרתי גלילה על פני הודעה**: הודעה אומרת "זה הצליח" ומשאירה אותך לחפש; הגעה לכרטיס מציבה אותך בדיוק במקום שבו הפעולה הבאה קורית — מילוי היום. ה-`highlight-pulse` הקיים אומר איזה מהכרטיסים הוא החדש, כך שגם מקבלים את מה שהודעה הייתה נותנת.
  - עובד גם אופליין: הכתיבה היא fire-and-forget, היום מגיע מהמטמון המקומי והאפקט מגלגל אליו כשהוא באמת מרונדר.
- **תצוגת סיכום ללוח הזמנים היומי**: שורה אחת ליום במקום ציר זמן מלא — כמה פעילויות, ושתי הראשונות עם השעה. בטיול של שבוע התצוגה המלאה היא כמה מסכי גלילה כדי לענות על "מה יש ביום חמישי?". **71px לכרטיס במקום 457px**; שישה ימים נכנסים למסך אחד.
  - לחיצה על יום פותחת אותו לציר הזמן המלא במקום; המצב נשמר בנפרד מ-`collapsedDays`, כך שמעבר בין התצוגות לא מבלבל את השנייה.
  - הבחירה נשמרת ב-`localStorage` תחת `dayLayout_<tripId>`.
  - ידית הגרירה נשארת בתצוגת הסיכום — סידור מחדש של ימים קל יותר דווקא כשרואים את כולם.
  - המתג הוא כפתור אייקון: לכותרת כבר יש שם ועד שתי פעולות עם תווית, ותווית שלישית דחפה את הכותרת לשורה שנייה.

## [8.11.2] - 2026-09-06 — "ימים מסונכרנים" נעלם מעצמו

### Fixed
- **ההודעה הירוקה "ימים מסונכרנים עם הטיסה" הייתה קבועה** (`PlanningTab.jsx`): היא רונדרה מהשוואה שמורה בין תאריכי הטיסה ל-`plannerDaysFromFlight`, שנכונה מרגע הסנכרון ועד שהתאריכים משתנים — כלומר לתמיד, בפועל. הודעה שלא עוזבת מפסיקה להיקרא.
  - עכשיו זו **הודעת אישור ולא סטטוס**: היא מופיעה רק כשסנכרון באמת רץ, ונעלמת אחרי 4 שניות.
  - מה שהיא ייצגה כבר נמצא על הכפתור שלידה, שנעלם כשהימים מסונכרנים ומופיע כ"עדכן ימים לפי טיסה" כשהם לא.
  - נוספה הנפשה קצרה (הופעה, החזקה, דעיכה) מתוזמנת לאותן 4 שניות, כדי שהיא תצא בעצמה ולא תיעלם בהבזק.
  - **`prefers-reduced-motion`**: הבלוק הגלובלי מקצר כל הנפשה ל-0.01ms, ו-`forwards` היה מקבע אז את הפריים **האחרון** — כלומר `opacity: 0` — ומסתיר את ההודעה בדיוק ממי שביקש פחות תנועה, לא פחות מידע. יש override מפורש שנותן להם אותה סטטית.
  - אומת מקצה לקצה: לא מוצגת לפני סנכרון, מוצגת מיד אחריו (5 ימים נוצרו), עדיין ב-2.6 שניות, נעלמה ב-5, **ולא חוזרת ביציאה וכניסה חוזרת לטאב** — שזה היה הבאג עצמו.

## [8.11.1] - 2026-08-27 — מאזין אחד למסמך הטיול

### Fixed
- **`PlanningTab` פתח שני מאזינים לאותו מסמך טיול בדיוק** — שני `useEffect` עם אותן תלויות, כל אחד קורא שדות אחרים מאותו doc. כל עריכה של הטיול העירה שתי מנויים, שתי callbacks ושני מחזורי רינדור, ו-Firestore החזיק שני streams פתוחים למסמך אחד. אוחדו לאחד.
  - אומת ששני הצדדים עדיין מגיעים: `hotelDetails`/`distanceOrigins` (רשימת המקומות נטענת), ו-`tripFlightDates` (כפתור "ייצר ימים לפי תאריכי הטיסה" מופיע כשאין ימים) — הצד השני הוא בדיוק מה שהמאזין המיותר האכיל.

## [8.11.0] - 2026-08-27 — מי שעזב את הטיול

### Added
- **`trips/{id}.formerMembers`** — תמונת מצב (שם, אימייל, תמונה, מועד עזיבה) שנכתבת ברגע שמישהו יוצא מטיול או מוסר ממנו.
  התוכן עצמו **מעולם לא נמחק** ביציאה: `leaveTrip` ו-`removeMember` נגעו רק ב-`members` וב-`memberIds`, וכל מה שהאדם הוסיף שייך לטיול ולא לו. אבל פרופילים נטענים **רק** עבור חברים נוכחיים, ולכן מרגע שה-uid יצא מ-`members` כל הוצאה ששילם וכל פריט שהיה משויך אליו הוצגו בלי שם בכלל. התמונה הזו היא מה שמציל את הייחוס.
- **`person-former`**: מי שעזב מוצג באפור ומחוויר (`grayscale`) בכל מקום שבו הוא מופיע — האווטאר בשורת הפריט, שבב הסינון בצ׳קליסט, המשלם בשורת ההוצאה ובסיכום "לפי משלם" — עם `title` שמסביר "כבר לא בטיול". התוכן שלו נשאר בדיוק במקומו.

### Changed
- **מי שעזב עדיין מופיע ברשימות — אבל רק אם משהו עדיין תלוי בו.** מי שעזב בלי שום שיוך פשוט נעלם; מי שעדיין מחזיק שורות נשאר ברשימה כדי שלשורות האלה יהיה שם ויהיה אפשר למצוא אותן דרך שבב הסינון.
- **אי אפשר לשייך מחדש למי שעזב**: בטופס הוספת פריט הוא לא מוצע כלל, ובבורר השיוך הוא מופיע רק אם הפריט כבר שלו — כדי שאפשר יהיה להעביר אותו הלאה.
- **מי שעזב לפני שהתמונה הזו הייתה קיימת** מוצג כ"משתתף שעזב" במקום "?" — השם אבד, ההוצאה לא.
- **נוסח היציאה מטיול** אומר עכשיו במפורש מה קורה לתוכן: "מה שהוספת — הוצאות, מקומות, פריטים ברשימה — נשאר בטיול על שמך".
